...
| Service | URL/Config | Statut | Utilisé par |
|---------|------------|--------|-------------|
| **Dolibarr** | `dolibarr.coolify.daniltech.com` | ✅ Fonctionnel | Facturation, clients |
| **n8n** | Workflow automation | ✅ Configuré | Automatisations |
| **Paperless-ngx** | Gestion documents | ⚠️ 401 (à configurer) | Archivage |
| **NeuronWriter** | SEO content | ⚠️ 401 (à configurer) | Optimisation contenu |
| **WordPress** | `bijouteriehidous.com` | ✅ Lecture/Écriture | Site web, blog |
| **WooCommerce** | E-commerce | ❌ Non configuré | Produits, commandes |
| **Cloudflare** | DNS + Cache | ✅ Fonctionnel | Performance, sécurité |
| **Google Analytics 4** | Property 334819686 | ✅ Fonctionnel | Analytics, rapports |

---

## 🛠️ Maintenance OpenClaw - Règles d'or

### ⚠️ AVANT chaque mise à jour
1. **Backup manuel** : `/data/workspace/maintenance-openclaw.sh`
2. **Vérifier état** : `openclaw doctor --non-interactive`
3. **Dans Coolify** : changer UNIQUEMENT le numéro de version
4. **JAMAIS** changer de registry (garder `coollabsio/`)

### 📋 Checklist santé (1x/semaine)
- [ ] `openclaw doctor --non-interactive` → pas d'erreurs
- [ ] `du -sh ~/.openclaw/agents` → < 50Mo
- [ ] Permissions credentials → 600
- [ ] Backup récent dans `/data/workspace/backups/openclaw/`

### 🆘 En cas de problème
**Boucle de redémarrage** → Restaurer snapshot Coolify immédiatement
**Données manquantes** → Restaurer depuis `/data/workspace/backups/openclaw/`

---

## 🔌 Connexions API - État détaillé

### ✅ Fonctionnelles
| Service | Auth | Détail |
|---------|------|--------|
| Dolibarr | `DOLAPIKEY` | Clé API header |
| Cloudflare | API Token | Account + Zone ID configurés |
| Telegram | Bot Token | @NeoHidousBot, allowlist Danil |
| GA4 | Service Account | `neo-seo-agent@neo-488103.iam.gserviceaccount.com` |
| WordPress | App Password | User `neo`, rôle Éditeur |

### ⚠️ En cours de configuration
| Service | Problème | Action requise |
|---------|----------|----------------|
| NeuronWriter | 401 Non autorisé | Vérifier header `X-API-KEY` |
| Paperless-ngx | 401 Non autorisé | Vérifier header `Authorization: Token` |
| WooCommerce | Non configuré | Créer Consumer Key + Secret |
...
| Fichier | Description |
|---------|-------------|
| `AGENTS_SEO_REFERENCE.md` | Tous les agents, LLM, connexions API |
| `BIJOUTERIE_HIDOUS.md` | Fiche entreprise complète |
| `MAINTENANCE_OPENCLAW.md` | Procédures maintenance et MAJ |
| `MEMORY.md` | Ce fichier - mémoire long terme |
| `cron-jobs-optimized.json` | Jobs cron actifs |
| `.learnings/ERRORS.md` | Journal des erreurs |
| `.learnings/LEARNINGS.md` | Leçons apprises |
...
### 2026-03-15 - Vérification versions OpenClaw & retrait Straico
**Version actuelle :** v2026.3.8  
**Dernière version GitHub :** v2026.3.13-1  
**Écart :** 5 versions en retard

**Changements importants dans v2026.3.13 :**
- Fix compaction : vérification améliorée des tokens post-compaction
- Fix agents : éviter l'injection double des fichiers mémoire
- Docker : support `OPENCLAW_TZ` pour fuseau horaire
- UI : améliorations mobile et thèmes
- Fix : préservation persona/langue dans résumés de compaction
- Browser : normalisation batch pour selectors

**Retrait de Straico :**
- Clé API invalide détectée lors des tests
- Fournisseur retiré de la configuration
- Impact : modèles GPT-5.2, Claude 4.5, Gemini 3 Flash, etc. via Straico indisponibles
- Fallbacks automatiques sur NVIDIA/Moonshot/OpenRouter fonctionnels

**TODO :** Mettre à jour OpenClaw vers v2026.3.13-1 (nécessite restart gateway)

### 2026-03-15 - Complétion mémoire avec informations manquantes
**Sources ajoutées :** AGENTS_SEO_REFERENCE.md, BIJOUTERIE_HIDOUS.md, MAINTENANCE_OPENCLAW.md

**Nouvelles sections ajoutées dans MEMORY.md :**
- Stack technique complète (n8n, Dolibarr, Paperless-ngx, NeuronWriter)
- Procédures de maintenance OpenClaw
- Checklist santé hebdomadaire
- Règles d'or pour les mises à jour
- Informations WordPress (lecture/écriture)
- Connexions API en cours de configuration (NeuronWriter, Paperless, WooCommerce)
- Agents cron détaillés (local-seo, analytics-seo, keyword-tracker)
- Fichiers de référence centralisés
